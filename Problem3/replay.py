...
def validate(name):
    ...
    checks={'soc_reconstruction_kwh':float(np.abs(e-reconstructed).max()),
        ...
        'executed_template_mapping_kwh':float(np.abs(q[1:].reshape(days,144)-qf).max()),
        'lp_residual':float(a['lp_residual'])}
    for r in range(3):assert np.max(np.abs(delta[:,r,:(r+1)*36-1]),initial=0)<1e-7
    ...
    trade=original+incr
    alternate=(qf*row_price).sum(1)+(.5*np.abs(delta)*row_price[:,None,:]).sum((1,2))
    checks['settlement_identity_yuan']=float(np.abs(trade-alternate).max())
    if any(v>1e-5 for v in checks.values()):raise AssertionError(checks)
    # Natural calendar evaluation and shifted template rows are both reported.
    start=31*144;end=days*144
    emergency_calendar=float((h[start:end]*5*p[start:end]).sum())
    emergency_template=float((h[start+1:end+1]*5*p[start+1:end+1]).sum())
    cost_at=np.zeros(n);cost_at[0]=q[0]*p[0]
    cost_at[1:]=(q0*row_price+((1.5*np.maximum(delta,0)-.5*np.maximum(-delta,0))*row_price[:,None,:]).sum(1)).reshape(-1)
    cost_calendar=float(cost_at[start:end].sum())+emergency_calendar
    cost_template=float(trade[31:].sum())+emergency_template
    ...
    # Additional no-refund ledger sensitivity for exactly the same actions.
    # It is NOT a reoptimized solution under that alternative interpretation.
    no_refund_increment=float((np.maximum(-delta[31:],0)*row_price[31:,None,:]).sum())
    report={'variant':name,'days':days,'checks':checks,'initial_feb_soc_kwh':float(e[31*144]) if days>31 else None,
        'calendar_cost_yuan':cost_calendar,'template_cost_yuan':cost_template,
        'planned_cost_yuan':float(original[31:].sum()),'adjustment_net_cost_yuan':float(incr[31:].sum()),
        'calendar_emergency_cost_yuan':emergency_calendar,'template_emergency_cost_yuan':emergency_template,
        'calendar_emergency_kwh':float(h[31*144:days*144].sum()),
        'template_emergency_kwh':float(h[31*144+1:days*144+1].sum()),
        ...
        'same_actions_no_refund_extra_cost_yuan':no_refund_increment,'paper_dates':paper,
...
def verify_workbook():
    ...
    # Hand-worked incremental settlement check: 100 -> 80 -> 90 at price 1.
