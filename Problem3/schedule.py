...
def run(name,updates,f,count,days):
    ...
    # Initial unobserved ten minutes: declared reference-profile warmup.
    inherited=max(0,truth[0]/6)
    for day in range(days):
        for r in range(4):
            k=day*4+r;start=k*36;periods=np.arange(start,start+145)
            pp=price[periods%144];paths,ids=scenarios(f,k,count);chosen[day,r]=ids
            if r==0:
                q,res=lp(paths,pp,e[start],fixed_first=inherited);worst=max(worst,res)
                q0[day]=q[1:145];final[day]=q0[day]
            elif r*6 in updates:
                offset=r*36-1;old=final[day,offset:].copy()
            ...
            for t in range(start,start+36):
                offset=t-day*144
                executed[t]=inherited if offset==0 else final[day,offset-1]
                c[t],d[t],h[t],w[t],e[t+1]=p2.feedback(executed[t],truth[t]/6,e[t])
        inherited=final[day,-1]
        if day%30==0 or day==days-1:
            print(name,str(data.dates[day]),'seconds',round(time.perf_counter()-started,1),flush=True)
    # Last template cell is Jan1 2026 00:00-00:10 (or the pilot end boundary).
    executed[-1]=inherited
    c[-1],d[-1],h[-1],w[-1],e[-1]=p2.feedback(inherited,truth[n-1]/6,e[-2])
